...
CREATE PROCEDURE [dbo].[usp_LoadDimaccountInfo_10212020] AS
/******************************************************************************        
...
BEGIN 
 ...
 --   DELETE  FROM #T1  WHERE CPG_DivisionID='n/a' and RetailerID=5 and ProductType_ID=4 and CPG_AccountID='0016000000Ph1SKAAZ' 
	...
	UPDATE T SET
		...
		UPDATE a SET 
			[NATIONAL/RetailerOnly] = CASE WHEN [CPG_AccountID] ='n/a' AND RetailerID <>18 THEN 'Retailer only' ELSE 'National' END 
		FROM [MSTRReportingDB].[dbo].[DimAccountInfo_New]  a
	

		UPDATE a SET 
			[CPG/Shopper_Inv] = CASE WHEN  RetailerID = 18 THEN 'National Only'
